...
namespace ryujin
{
  namespace MultiSpeciesEuler
  {
    ...
    template <int dim, typename Number>
    DEAL_II_ALWAYS_INLINE inline Number
    RiemannSolver<dim, Number>::alpha(const Number &gamma,
                                      const Number &a) const
    {
      const Number numerator = ScalarNumber(2.) * a;
        ...
        const primitive_type &riemann_data_j) const
    {
      ...
      const auto alpha_i = alpha(gamma_i, a_i);
      const auto alpha_j = alpha(gamma_j, a_j);
      ...
      /*
       * The admissible set is p_min >= 0. But numerically let's avoid
      ...
      const Number p_1_tilde =
          p_max * ryujin::pow(safe_division(numerator, first_denom),
      ...
      const Number p_2_tilde =
          p_max * ryujin::pow(safe_division(numerator, second_denom),
        ...
        const primitive_type &riemann_data_j) const
    {
      ...
      const Number alpha_hat_i = c(gamma_i) * alpha(gamma_i, a_i);
      const Number alpha_hat_j = c(gamma_j) * alpha(gamma_j, a_j);
      ...
      const Number denominator =
          alpha_hat_i * ryujin::pow(safe_division(p_i, p_j), -exponent) +
          alpha_hat_j;
        ...
        const primitive_type &riemann_data_j) const
    {
      ...
      const Number radicand_i =
          safe_division(ScalarNumber(2.) * p_max,
        ...
        const primitive_type &riemann_data_j) const
    {
      ...
      const Number p_tilde = p_max * ryujin::pow(temp, exponent_inverse);
                                  ...
                                  const Number p_star) const
    {
      ...
      const Number radicand = safe_division(Az, p_star + Bz);
      ...
      const Number ratio = safe_division(p_star, p);
      const Number factor = ryujin::pow(ratio, exponent) - Number(1.);

      /* false_value is rarefaction case */
      const auto false_value = ScalarNumber(2.) * a * factor /
        ...
        const primitive_type &riemann_data, const Number p_star) const
    {
      ...
      const Number tmp = safe_division(positive_part(p_star - p), p);
        ...
        const dealii::Tensor<1, dim, Number> &n_ij) const -> primitive_type
    {
      ...
      const auto gamma = view.gamma_mixture(U);
        ...
        const dealii::Tensor<1, dim, Number> &n_ij) const
    {
      ...
      const auto riemann_data_j =
          riemann_data_from_state(U_j, p_j, rho_j, n_ij);
