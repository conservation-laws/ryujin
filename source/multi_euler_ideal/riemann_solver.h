...
namespace ryujin
{
  namespace MultiSpeciesEuler
  {
    ...
    /**
     * A fast approximate solver for the 1D Riemann problem. The solver
     * ensures that the estimate \f$\lambda_{\text{max}}\f$ that is returned
     * for the maximal wavespeed is a strict upper bound.
     *
     * The solver is based on the guaranteed upper bound wavespeed estimates
     * derived in @cite ClaytonGuermondPopov-2022. The key idea is to
     * compute an upper bound on the intermediate pressure \f$p^*\f$ in the
     * Riemann problem and then use this to derive guaranteed upper bounds
     * on the wavespeeds \f$\lambda_1^-\f$ and \f$\lambda_3^+\f$.
     *
     * For the multi-species case, we use a surrogate gamma
     * \f$\gamma_{\min}\f$ computed as the minimum over the stencil to
     * ensure the wavespeed bounds remain valid. The maximum wavespeed is
     * then (see Eq. (3.3) in [ClaytonDzanicTovar-2025]):
     * \f[
     *   \lambda_{\max} = \max(|\lambda_1^-|, |\lambda_3^+|),
     * \f]
     * where \f$\lambda_1^-\f$ and \f$\lambda_3^+\f$ are computed using the
     * surrogate EOS with \f$\gamma_{\min}\f$.
     *
    ...
    protected:
      ...
      /**
       * Continuous and monotonic function c(gamma) as defined in (A.3)
       * on page A469 of @cite ClaytonGuermondPopov-2022.
       *
      ...
      /**
       * Compute alpha = 2a / (gamma - 1).
       *
       * Cost: 0x pow, 1x division, 0x sqrt
       */
      Number alpha(const Number &gamma, const Number &a) const;
...
#ifndef DOXYGEN
      /*
       * Helper function f for Riemann solver.
       */
      Number f(const primitive_type &riemann_data, const Number p_star) const;


      /*
       * Helper function phi for Riemann solver.
       */
       ...
       * monotone increasing in p, concave down and whose (weak) third
