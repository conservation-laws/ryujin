  ...
  namespace ShallowWater
  {
    ...
    public:
      ...
      using flux_contribution_type =
          std::tuple<state_type<dim, Number>, Number>;
      ...
      static Number
      water_depth(const dealii::Tensor<1, problem_dim, Number> &U);
      ...
      template <typename ST,
                ...
                typename T = typename ST::value_type>
      ST
      affine_shift_stencil_source(const std::tuple<ST, T> &prec_i,
                                  const std::tuple<ST, T> &prec_j,
                                  const T &d_ij,
                                  const dealii::Tensor<1, dim, T> &c_ij) const;
    ...
    template <int problem_dim, typename Number>
    DEAL_II_ALWAYS_INLINE inline Number HyperbolicSystem::water_depth(
        const dealii::Tensor<1, problem_dim, Number> &U)
    {
      return U[0];
    }
        ...
        const dealii::Tensor<1, problem_dim, Number> &U) const
    {
      ...
      const Number h_cutoff_mollified =
          Number(reference_water_depth_ * dry_state_relaxation_mollified_) *
          eps;
    ...
    template <int problem_dim, typename Number>
    DEAL_II_ALWAYS_INLINE inline Number HyperbolicSystem::water_depth_sharp(
        const dealii::Tensor<1, problem_dim, Number> &U) const
    {
      using ScalarNumber = typename get_value_type<Number>::type;
      constexpr ScalarNumber eps = std::numeric_limits<ScalarNumber>::epsilon();
