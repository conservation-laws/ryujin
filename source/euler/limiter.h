...
namespace ryujin
{
  namespace Euler
  {
    ...
    public:
      ...
      {
        return parameters_.value().iterations;
      }
      ...
      {
        return ScalarNumber(parameters_.value().relaxation_factor);
      }
    ...
    public:
      ...
      LimiterView(const View &view, const Limiter<ScalarNumber> &limiter)
          : view_(view)
          , parameters_(limiter.parameters_.template get_view<MemorySpace>())
      {
      }
    ...
    private:
      ...
      using ParameterView =
          typename Mirrored<typename Limiter<ScalarNumber>::Parameters>::
