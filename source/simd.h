...
#ifndef DOXYGEN
  /* Host-only implementation of pow() below, defined in simd.cc: */
  template <typename T>
  T pow_impl(const T x, const T b);
#endif


  /**
   * Custom serial pow function. On the host we use a custom
   * implementation based on the vectorclass library, on the device we
   * simply call the native std::pow() function.
   *
   * @ingroup SIMD
   */
  template <typename T>
  DEAL_II_HOST_DEVICE_ALWAYS_INLINE T pow(const T x, const T b)
  {
    KOKKOS_IF_ON_DEVICE((return std::pow(x, b);))
    KOKKOS_IF_ON_HOST((return pow_impl(x, b);))
  }
