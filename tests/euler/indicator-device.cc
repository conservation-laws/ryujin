...
{
  //
  // Test that the IndicatorView can be used on the device memory space:
  //
  ...
  ryujin::Euler::Indicator<double> indicator(hyperbolic_system);
  ...
  for (unsigned int i = 0; i < n_states; ++i) {
    ...
    for (unsigned int j = 0; j < n_states; ++j) {
      ...
      c_ij[0] = 0.03 * (double(j) - double(i));
      c_ij[1] = 0.01 * (double(j) + 1.);
      view.accumulate(pv, js, U_j, c_ij);
    }
    std::cout << view.alpha(hd_i) << "\n";
  }

  /* Compute alpha_i on the default space: */
  ...
  Kokkos::parallel_for("test",
                       ...
                       [=](std::size_t i) {
                         ...
                         for (unsigned int j = 0; j < n_states; ++j) {
                           ...
                           c_ij[0] = 0.03 * (double(j) - double(i));
                           c_ij[1] = 0.01 * (double(j) + 1.);
                           view.accumulate(pv_view, js, U_j, c_ij);
                         }
                         dealii::Tensor<1, 1, double> result;
                         result[0] = view.alpha(hd_i);
  ...
  for (unsigned int i = 0; i < n_states; ++i)
    std::cout << results.read_tensor<double>(i)[0] << "\n";
}
